...
import com.example.jobApplication.Entity.Repository.JobDataRepo;
        ...
        for (int j = 0; j < 3; j++) {
            ...
            for (int i = 0; i < driver.findElements(JOB_CARD_LOCATOR).size(); i++) {
                ...
                try {
                    ...
                    JobDataRepo.save(jobData);
                    randomDelay(1200, 2500);

                } catch (StaleElementReferenceException ignored) {
                    // DOM refreshed — safe to skip
                } catch (DataIntegrityViolationException | JDBCException e) {
                    System.err.println("DB error — skipping job: " + e.getMessage());
                } catch (TimeoutException | NoSuchElementException e) {
                    System.err.println("DOM error — skipping job card");
                } catch (Exception e) {
                    System.err.println("Unexpected error — skipping job");
                }
            }
        }
        return jobsList;
